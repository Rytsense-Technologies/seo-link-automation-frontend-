...
function renderDetail({ id = SUGGESTION.id, backHref = "/interlink" }: { id?: string; backHref?: string } = {}) {
...
function serve(detail: Partial<SuggestionDetailData> = {}) {
...
describe("SuggestionDetail", () => {
  it("shows a loading skeleton, not a blank page, while the request runs", async () => {
    serve();
    renderDetail();
    expect(screen.getByRole("heading", { level: 1, name: "Suggestion Detail" })).toBeInTheDocument();
    expect(screen.getByRole("status", { name: "Loading suggestion" })).toBeInTheDocument();
    expect(await screen.findByRole("region", { name: "Link suggestion" })).toBeInTheDocument();
  ...
  it("renders the full suggestion from the detail endpoint only", async () => {
    const api = serve();
    renderDetail();
    await screen.findByRole("region", { name: "Link suggestion" });

    const header = screen.getByRole("banner");
    expect(within(header).getByText("Pending")).toBeInTheDocument();
    expect(within(header).getByRole("meter", { name: "Relevance score" })).toHaveAttribute("aria-valuenow", "55");
  ...
  it("shows the anchor text and the exact context, with the anchor highlighted", async () => {
    serve();
    renderDetail();
    const suggestion = await screen.findByRole("region", { name: "Link suggestion" });
  ...
  it("shows relevance score, retrieval score and reason as returned", async () => {
    ...
    expect(within(relevance).getByText("0.5487")).toBeInTheDocument();
    expect(
      within(relevance).getByText("Line one. Line two.", { normalizer: (text) => text.replace(/\s+/g, " ") }),
    ).toBeInTheDocument();
  });
  ...
  it("shows a deterministic suggestion without inventing AI details", async () => {
    ...
    renderDetail();
    await screen.findByRole("region", { name: "Link suggestion" });
    const generation = section("Generation");

    expect(within(generation).getByText("deterministic")).toBeInTheDocument();
    const model = within(generation).getByText("Model").nextElementSibling;
  ...
  it("shows the provider and model an AI suggestion records", async () => {
    ...
    renderDetail();
    await screen.findByRole("region", { name: "Link suggestion" });
    expect(within(section("Generation")).getByText("groq")).toBeInTheDocument();
    expect(within(section("Generation")).getByText("llama-3.3-70b")).toBeInTheDocument();
  });
