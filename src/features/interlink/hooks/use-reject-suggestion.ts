...
export function rejectBody(reason: string | null | undefined): { reason?: string } {
  const trimmed = reason?.trim() ?? "";
  return trimmed ? { reason: trimmed } : {};
}

/** `POST /interlink/suggestions/{id}/reject` with an optional reason. */
export function rejectSuggestion(suggestionId: string, reason?: string | null): Promise<SuggestionDetail> {
  return apiRequest<SuggestionDetail>(`${suggestionDetailPath(suggestionId)}/reject`, {
    method: "POST",
...
export function useRejectSuggestion(suggestionId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (reason: string | null) => rejectSuggestion(suggestionId, reason),
