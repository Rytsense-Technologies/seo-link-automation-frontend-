...
describe("apiRequest", () => {
  it("returns parsed JSON on success", async () => {
    ...
    const result = await apiRequest<{ total: number }>("interlink/suggestions", { query: { status: "PENDING" } });
  ...
  it("throws an ApiError carrying the backend code", async () => {
    ...
    const error = await apiRequest("interlink/suggestions/abc/apply", { method: "POST" }).catch((e: unknown) => e);
  ...
  it("turns a failed fetch into a network ApiError", async () => {
    ...
    const error = await apiRequest("interlink/suggestions").catch((e: unknown) => e);
